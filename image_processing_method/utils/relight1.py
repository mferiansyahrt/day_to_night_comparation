...
class LocalLight:
    ...
    def __init__(self, id_, color, location, size, scale, ambient=False, sat=True):
        self.id = id_
        self.color = color
...
def relight_locally(image, illuminants,illuminants_low_temp, cfa_pattern, clip=False, invert_wb=True, min_light_size=0.5, max_light_size=1.0, num_sat_lights=1):
    """
    ...
    for i in range(len(illuminants)):
        if i == (len(illuminants ) - (num_sat_lights + 1)):
            light = generate_low_temp_light(id_=i, illuminant=illuminants_low_temp[i],image_shape=image.shape,min_light_size=min_light_size, max_light_size=max_light_size, scale=1.0,ambient=False, sat=True)  # first light is ambient, last num_sat_lights are saturated
        else:
            light = generate_random_light(id_=i, illuminant=illuminants[i],image_shape=image.shape,min_light_size=min_light_size, max_light_size=max_light_size, scale=1.0,ambient= i==0, sat=len(illuminants) )
...
# first scaled ambient light is applied
    ...
    #image_relight = apply_local_lights_rgb(image, local_lights, clip, invert_wb)
    return image_relight, local_lights


def apply_local_lights(image, local_lights, cfa_pattern, clip, invert_wb=True, num_sat_lights=2):
    """
    ...
    np.array(image_relights[:len(local_lights)-(num_sat_lights+ 1)]),
    axis=0, 
    weights=weights[:len(local_lights)-(num_sat_lights+ 1)]
)

    for ll in range(len(local_lights)- ( num_sat_lights + 1 ),len(local_lights)):
        image_relight += 1*weights[ll, :, :] * image_relights[ll]
...
def generate_random_light(id_, illuminant, image_shape, min_light_size, max_light_size, scale,
                          ambient, sat):
    """
...
def generate_low_temp_light(id_, illuminant, image_shape, min_light_size, max_light_size, scale,
                          ambient, sat):
    """
    Generate a local light with random location and size.
    :param id_: ID.
