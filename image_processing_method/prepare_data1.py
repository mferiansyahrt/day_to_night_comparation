...
def parse_args():
    ...
    parser.add_argument('--relight_local', default=True, action='store_true', help='locally relight or not')
...
def synth_night_imgs(wb_random,in_img_or_path, in_day_meta_data, gt_illum_mean , 
                                            gt_illum_cov , dim, relight, iso,
                     discard_black_level, _clip, relight_local,
                     min_num_lights, max_num_lights, min_light_size, max_light_size , num_sat_lights):
    ...
    if relight_local:
        ...
        if discard_black_level:
            ...
            if wb_random:
                print("wb_random : true \n")
                sampled_wb = np.random.multivariate_normal(gt_illum_mean, gt_illum_cov, num_of_samples)
                #sampled_wb_lowtemp = sampled_wb
                #sampled_wb_lowtemp[:,2] = sampled_wb_lowtemp[:,2] * 1.1 
            else:
                print("wb_random : false \n")
                sampled_wb = np.array([[0.59639085,0.5,0.05671734],
    ...
    if relight_local:
        ...
        print(sampled_wb)
        sampled_wb_lowtemp = sampled_wb
        print("sampled_wb low temp\n")
        print(sampled_wb_lowtemp)
        #blue
        sampled_wb_lowtemp[4:5,2] = sampled_wb_lowtemp[4:5,2] * 0.9 
        sampled_wb_lowtemp[4:5,1] = sampled_wb_lowtemp[4:5,1] * 0.9 
        #red
        sampled_wb_lowtemp[5:7,0] = sampled_wb_lowtemp[5:7,0] * 1.1 
        #green
        sampled_wb_lowtemp[5:7,1] = sampled_wb_lowtemp[5:7,1] * 0.95
        print("sampled_wb low temp after multiplication \n")
        print(sampled_wb_lowtemp)
        white_imbalanceed_img_dim_night, local_lights_ = relight_locally(white_balanceed_img_dim, sampled_wb,sampled_wb_lowtemp,
                                                                         in_day_meta_data['cfa_pattern'], clip=_clip,
                                                                         invert_wb=True,
...
if __name__ == "__main__":
    ...
    for example_num in range(len(outdoor_daytime_img_names)):
        ...
        results_ = synth_night_imgs(example_img_bayer_org, meta_data_org, dim=args.dim,
                                    ...
                                    num_sat_lights=args.num_sat_lights,gt_illum_mean = gt_illum_mean, 
